...
class SVMStruct:
    def __init__(self, dataSet, labels, C, toler, kernelOption):
        ...
        self.numSamples = dataSet.shape[0]  # number of samples 有多少行训练数据
        self.alphas = mat(zeros((self.numSamples, 1)))  # Lagrange factors for all samples拉格朗日乘子，初始化为0
        self.b = 0
        self.errorCache = mat(zeros((self.numSamples, 2))) #差错，这样就不用每次都求了
        self.kernelOpt = kernelOption            #核函数选择
        self.kernelMat = calcKernelMatrix(self.train_x, self.kernelOpt)  #利用核函数
...
def testSVM(svm, test_x, test_y):
    ...
    for i in range(numTestSamples):
        ...
        if(i==0):
            print(kernelValue)
            print(multiply(supportVectorLabels, supportVectorAlphas))

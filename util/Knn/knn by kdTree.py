...
def preoderByRecursion(node):
    if node != None:
        print("%s,%s" % node.data, node.split)
...
def classify(path,inX,k): # path是遍历到的节点，inX是待确认点，k表示根据周围k个节点来进行分类
    ...
    for index in numpy.arange(0,k):
        classData[path[index].type]=classData.get(path[index].type,0)+1
    return sorted(classData,key=lambda x:classData[x])[-1] #返回确认类型


if __name__ == "__main__":
    inX = input("请输入要确认的数inX：")
    k = int(input("请输入k值："))
    inX = inX.split(' ')
    ...
    # print(headNode.data)
    # print(headNode.leftChild.data)
    # print(headNode.rightChild.data)
    # print(headNode.leftChild.leftChild.data)
    # print(headNode.leftChild.rightChild.data)
    # print(headNode.rightChild.leftChild.data)
    # print(headNode.rightChild.rightChild.data)
    # print(headNode.leftChild.leftChild.leftChild.data)
    # print(headNode.leftChild.rightChild.leftChild.data)
    # print(headNode.rightChild.leftChild.leftChild.data)
    # print(headNode.rightChild.rightChild.leftChild.data)
